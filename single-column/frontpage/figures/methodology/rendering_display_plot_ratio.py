...
def do_plot_time(files_list, param):
    ...
    ax2.set_title('Render to plt ratio', fontsize=8)
    ax2.set_xlabel('Ratio')
    ax2.set_ylabel('CDF')
    line_1.set_label('Non-mobile render to plt ratio')
    #line_2.set_label('comp-b20-d50')
    #line_3.set_label('comp-b20-d150')
    line_4.set_label('Mobile render to plt ratio')
    ...
    plt.show()
    plt.savefig('../plots/ratio/new/' + 'render_to_plt_ratio'  + ".pdf")
    plt.close()
...
def main():
    ...
    for files in dirs:
        if files.endswith('.bench'):
            ...
            #d = (a / c)
            #e = (b/ c)
            file1 = make_cdf(sorted(b), 'render_ratio_' + name1)
